...
sys.path.append('/data/data1/yixing/code/msra/msra-exp1/lost-in-the-middle/utils_test/mods_llama')
from mod_modeling_llama import mod_LlamaDecoderLayer_forward, mod_LlamaModel_forward
from mod_attention import mod_Longchat_Attention_forward, mod_LlamaAttention_forward
...
def main(
    ...
    ## yixing
    # model.model: LlamaModel(config)
    # model.model.layers = nn.ModuleList([LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
    # layer.self_attn = ( LlamaAttention(config=config)  if not getattr(config, "_flash_attn_2_enabled", False)
            # else LlamaFlashAttention2(config=config) ) 
    # layer.forward: modified!
    model.model.forward = MethodType(mod_LlamaModel_forward, model.model)
    for ith_layer, layer in enumerate(model.model.layers):
        model.model.layers[ith_layer].forward = MethodType(mod_LlamaDecoderLayer_forward, model.model.layers[ith_layer])
        
        # model.model.layers[ith_layer].self_attn.forward = MethodType(mod_Longchat_Attention_forward, model.model.layers[ith_layer].self_attn)
        model.model.layers[ith_layer].self_attn.forward = MethodType(mod_LlamaAttention_forward, model.model.layers[ith_layer].self_attn)
    ...
    # start: yixing
    kwargs_model = {'temperature': temperature, 'top_p': top_p} if do_sample else {}

    struct_time = time.localtime() 
    time_exp = time.strftime("%Y_%m_%d-%H_%M", struct_time) 

    save_attn_dir = f'/data/data1/yixing/code/msra/msra-exp1/lost-in-the-middle/utils_test/output/longchat-attn_score-nq_open-docu_{total_document}-gold_{gold_index}-longchat/{time_exp}/attn_score'
    model.model.save_attn_dir = save_attn_dir
    model.model.cur_prompt_id = None
    # end: yixing

    responses = []
    for ith_prompt, batched_prompts in tqdm(enumerate(chunks(prompts, batch_size)), total=math.ceil(len(prompts) / batch_size)):
        inputs = tokenizer(batched_prompts, return_tensors="pt", padding=True).to(model.device)

        # LlamaModel.cur_prompt_id
        model.model.cur_prompt_id = ith_prompt
        ...
        outputs = model.generate(
            **inputs,
            ...
            do_sample=do_sample,
            # temperature=temperature if do_sample else None, # yixing
            # top_p=top_p if do_sample else None, # yixing
            **kwargs_model, # yixing
