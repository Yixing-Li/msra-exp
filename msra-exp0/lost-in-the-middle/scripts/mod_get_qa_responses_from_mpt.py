...
sys.path.append('/data/data1/yixing/code/msra/msra-exp1/lost-in-the-middle/utils_test/mods')
from mod_modeling_MPT import mod_MPTForCausalLM_forward, mod_MPTModel_forward
from mod_block import mod_MPTBlock_forward
from mod_attention import mod_attn_forward, mod_triton_flash_attn_fn
# end: yixing
...
def main(
    ...
    # start: yixing
    # MethodType
    model.forward = MethodType(mod_MPTForCausalLM_forward, model)
    model.transformer.forward = MethodType(mod_MPTModel_forward, model.transformer)
    for ith_block, block in enumerate(model.transformer.blocks):
        # block: MPTBlock
        model.transformer.blocks[ith_block].forward = MethodType( mod_MPTBlock_forward, model.transformer.blocks[ith_block] )
        model.transformer.blocks[ith_block].attn.forward = MethodType( mod_attn_forward, model.transformer.blocks[ith_block].attn )
    # end: yixing
    ...
    save_attn_dir = f'/data/data1/yixing/code/msra/msra-exp1/lost-in-the-middle/utils_test/output/attn_score-nq_open-docu_{total_document}-gold_{gold_index}-m_3_i/{time_exp}/attn_score'
    model.save_attn_dir = save_attn_dir
    model.cur_prompt_id = None
    # end: yixing
    ...
    with torch.autocast(device, dtype=torch.bfloat16):
        for ith_prompt, batched_prompts in tqdm(enumerate(chunks(prompts, batch_size)), total=math.ceil(len(prompts) / batch_size)):
            ...
            model.cur_prompt_id = ith_prompt
