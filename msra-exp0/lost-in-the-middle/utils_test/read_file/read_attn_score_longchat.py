...
def main():
    ...
    test_block_list = [final_layer_idx]
    ...
    # 2023_12_20-12_00
    # 2023_12_18-14_19
    exp = 'longchat-attn_score-nq_open-docu_20-gold_4-longchat/2023_12_21-21_45'
    ...
    for root, dirs, files in os.walk(exp_data_dir):
            ...
            if 'prompt' in dir:
                ...
                for root_pt, dirs_pt, files_pt in os.walk(os.path.join(root, dir)):
                    for file_pt in sorted(files_pt, key = lambda x: int(x[x.find('-')+1:x.find('.npy')])):
                        ...
                        # get the attn score.
                        if cal_average:
                            if block_id == final_layer_idx:
                                got_attn_score = True
                                attn_score = np.load(os.path.join(root_pt, file_pt))
                                if (attn_score.shape[0] == 1):
                            ...
                            attn_score = np.squeeze(attn_score)
                            if absolute:
                                attn_score = np.absolute(attn_score)
                            x = np.arange(1, (attn_score.shape[-1]+1))
                            plt.bar(x, attn_score)

                            plot_str = f'Coordinates of Extreme Values:\nmax: ({attn_score.argmax()}, {attn_score.max():.2f})\nmin: ({attn_score.argmin()}, {attn_score.min():.2f})'
                            plot_cordinate = (2000, attn_score.max()-10)
                            print(attn_score)
        ...
        if cal_average and got_attn_score:
            ...
            plot_cordinate = (attn_min_x, attn_score_sum.max())
            bbox_props = dict(boxstyle="round", fc="w", ec="0.5", alpha=0.9)
            plt.text(plot_cordinate[0],plot_cordinate[1], plot_str, ha="center", va="center", size=10, bbox=bbox_props)

            plt.xlabel('token index')
            plt.ylabel('attn score')
            save_fig_dir = f'{out_dir}/avg_prompt_attn-{align_str}--layer_{final_layer_idx}.png'
