...
RMS_norm = RMSNorm(0, eps = 0, elementwise_affine = False)
len_tokens = 198
...
def plot_spectrum(a, exp_folder, exp_name, layer = 0, head = 0, ith_images = None, ith_image_ith = None):
    ...
    all_sqr = np.sum(s**2)
    hc_dc = (all_sqr - max_value ** 2) / max_value ** 2
...
def mod_Attention_forward(self, x, observe = False, ith_images = None):
    # great! got into here.
    # yixing-test
    ...
    if self.model_args['get_spectrum']:
        # ith_images
        ith_image_ith = 0
        hc_dcs = []
    ...
    if self.model_args['get_batch_simi']:
        batch_simi = compute_similarity(attn, attn) # one float for each block.
    ...
    # x = (attn @ v).transpose(1, 2).reshape(B, N, C)
    value = attn @ v
    if self.model_args['swish_GN'] and self.model_args['add_rmsnorm']:
